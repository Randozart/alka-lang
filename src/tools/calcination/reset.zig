// RESET — Emergency Power-State Reset Tool
//
// Purpose:
//   Bypasses OS power management to physically cut voltage to a component
//   via PCIe Power Management (PM) registers. RESET is the emergency
//   stop button — when thermal runaway or hardware malfunction is detected,
//   RESET cuts power before physical damage occurs.
//
// How it works:
...
//
// VITRIOL relevance:
//   GUARD triggers RESET when temperature exceeds HALT_AT. This is the
//   last line of defense — if thermal shadowing and YIELD fail to cool
//   the GPU, RESET physically cuts power to prevent damage.
//
// Op-Code: 0x1D
...
pub const RESET = struct {
    pub const OP = OpCode.RESET;
    pub const NAME = "RESET";
    pub const DESCRIPTION = "Reset subsystem to known state — physically cut voltage via PCIe PM";
